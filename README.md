...
| **技能子菜单** | 悬停（PC）或点击（移动端）技能图标展开二级子技能列表，继续悬停/点击可查看详细描述浮窗（全局唯一，同时仅展示一个） |
| **企业详情弹窗** | 点击企业卡片弹出毛玻璃风格详情弹窗，包含企业信息、核心业务场景流程图、核心技能徽章（点击/悬停可查看技能详情浮层）与合作推进进度条 |
| **底部信息面板** | 页面底部三个分类面板，各自展示对应技能分类下的企业滚动列表，支持展开/折叠 |
| **国际化（i18n）** | 支持中文 / 英文双语切换，全站文本响应式更新，无需刷新页面 |
| **设置面板** | 右上角齿轮按钮打开设置下拉菜单，可切换语言、移动端模式、底部面板显隐和待机演示 |
| **待机演示** | 可在设置面板中开启，5 秒无操作后自动循环 hover 随机企业卡片，鼠标活动即退出 |

### 视觉特性

- **毛玻璃效果（Glassmorphism）**：所有面板和弹窗采用 `backdrop-filter: blur()` + 半透明背景 + 发光边框，营造层次丰富的视觉效果
- **白色高光渐变光效**：标签、胶囊、弹窗边框等均采用 `color-mix(in srgb, color%, rgba(255,255,255))` 白色到彩色渐变发光，统一的高光风格
- **3D 等距视图**：中心技能平台通过三层预渲染切图（底座 → 中层底座 → 上层平台）叠加构成伪 3D 等距效果，底层平台独立 z-index 管理避免遮挡
- **SVG 发光连线**：使用 `feGaussianBlur` 高斯模糊滤镜与 `stroke-dasharray` 流光动画，实现动态发光连线效果
- **统一微动效**：所有可交互元素采用一致的 hover 放大（`scale(1.05~1.12)`）、亮度提升和 `cubic-bezier(0.4, 0, 0.2, 1)` 过渡曲线
- **渐变发光分割线**：弹窗标题与正文之间采用双层渐变背景（高亮线 + 扩散光晕），替代简单 border 实现发光分割效果

### 移动端适配

- 默认启用移动端模式，可在设置面板中手动切换
- 移动端下所有 hover 交互替换为 touch/click 事件：单击企业卡片激活高光状态，再次单击打开详情弹窗
- 技能节点点击展开子菜单，再次点击收起；子技能点击展示详情浮窗（Teleport 至 body，确保不被遮挡）
- 企业高亮状态下，未关联的技能节点不可点击；已展开子菜单中的暗淡节点点击后显示与高亮节点相同的发光效果
- 移动端专属关闭按钮：技能节点被点亮时，平台右上方出现毛玻璃风格关闭按钮，一键清除所有高亮状态
- 企业/技能高亮时 KPI 指标行禁用点击（`pointer-events: none`），防止误触
...
- `activeSubSkillDetailId`（当前展开详情的子技能 ID，全局唯一）

通过 props 向下传递、events 向上冒泡实现父子通信。跨层级通信（如 DOM 元素注册、全局子技能详情弹窗状态）使用 Vue 的 `provide/inject` 机制。全局设置状态（语言、设备模式等）通过 `useSettings` composable 以模块级单例模式共享。
...
子技能胶囊 点击
  ├─→ 切换详细描述浮窗显隐（Teleport 至 body 顶层）
  ├─→ 暗淡节点点击后显示与高亮节点相同的发光放大效果
  └─→ 全局唯一：点击新节点自动关闭前一个浮窗
...
| 类别 | 技术选型 | 说明 |
|------|----------|------|
| **前端框架** | Vue 3.5 | Composition API + `<script setup>` 语法 + Teleport |
